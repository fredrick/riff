(function() {
  ...
  function Dwolla(config, req) {
    ...
    this.avatar = function(accountKey) {
      return util.format('%s/avatars/%s', config['dwolla']['uri'], accountKey);
    };

    this.balance = function(token, callback) {
      return req.get({
        uri: util.format('%s/oauth/rest/balance', config['dwolla']['uri']),
        qs: {
          oauth_token: token
        },
        json: true
    ...
    this.user = function(token, callback) {
      return req.get({
        uri: util.format('%s/oauth/rest/users', config['dwolla']['uri']),
        qs: {
          oauth_token: token
        },
        json: true
    ...
    this.send = function(token, send, callback) {
      send['oauth_token'] = token;
      return req.post({
        uri: util.format('%s/oauth/rest/transactions/send', config['dwolla']['uri']),
